...
import java.lang.Runnable  // Thêm import cho Runnable
...
public class SpeechToTextPlugin :
    ...
    // Fix: Khai báo đầy đủ cho continuous mode
    private var shouldContinuousListen = false
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull rawResult: Result) {
        ...
        try {
            ...
            result.error(SpeechToTextErrors.unknown.name, "Unexpected exception", exc.localizedMessage)
        }
    }
    ...
    private fun hasPermission(result: Result) {
        ...
        if (localContext != null) {
            val hasPerm = ContextCompat.checkSelfPermission(localContext, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED
    ...
    private fun sdkVersionTooLow(): Boolean {
        return Build.VERSION.SDK_INT < minSdkForSpeechSupport
    }

    private fun isNotInitialized(): Boolean = !initializedSuccessfully

    private fun isListening(): Boolean = listening

    private fun isNotListening(): Boolean = !listening
    ...
    private fun startListening(result: Result, languageTag: String, partialResults: Boolean,
        ...
        val listenMode = enumValues<ListenMode>()[listenModeIndex]
    ...
    private fun stopListening(result: Result?) {  // Làm optional cho dummy calls
        shouldContinuousListen = false
    ...
    private fun locales(result: Result) {
        ...
        val hasPermission = ContextCompat.checkSelfPermission(pluginContext!!, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED
        if (Build.VERSION.SDK_INT >= 33 && hasPermission) {
            if (SpeechRecognizer.isOnDeviceRecognitionAvailable(pluginContext!!)) {
                val recognizer = createOnDeviceSpeechRecognizer(pluginContext!!)
                val recognizerIntent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH)
                recognizer?.checkRecognitionSupport(recognizerIntent, Executors.newSingleThreadExecutor(),
                    object : RecognitionSupportCallback {
                        override fun onSupportResult(recognitionSupport: RecognitionSupport) {
                            val details = LanguageDetailsChecker(result, debugLogging)
    ...
    private fun notifyListening(isRecording: Boolean) {
        ...
        if (!isRecording) {
            timer?.cancel()
            timer = null
            val doneStatus = if (!resultSent) SpeechToTextStatus.doneNoResult.name else SpeechToTextStatus.done.name
    ...
    private fun updateResults(speechBundle: Bundle?, isFinal: Boolean) {
        ...
        if (null != userSaid && userSaid.isNotEmpty()) {
            ...
            for (resultIndex in 0 until userSaid.size) {  // Fix: Use until để tránh index out
    ...
    private fun initializeIfPermitted(context: Context?) {
        ...
        permissionToRecordAudio = ContextCompat.checkSelfPermission(localContext, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED
        val permissionToEnableBluetooth = ContextCompat.checkSelfPermission(localContext, Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED
            ...
            if (localContext != null) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    if (!SpeechRecognizer.isRecognitionAvailable(localContext) && !SpeechRecognizer.isOnDeviceRecognitionAvailable(localContext)) {
                        ...
                        activeResult?.error(SpeechToTextErrors.recognizerNotAvailable.name, "Speech recognition not available on this device", "")
                        activeResult = null
                        return
                    }
                } else {
                ...
                activeResult?.error(SpeechToTextErrors.missingContext.name, "context unexpectedly null, initialization failed", "")
                activeResult = null
                return
            }
        }
    ...
    private fun createRecognizer(onDevice: Boolean, listenMode: ListenMode, pauseFor: Int?) {
        if (null != speechRecognizer && onDevice == lastOnDevice) return
        ...
        handler.post {
            ...
            if (null == speechRecognizer) {
                ...
                activeResult?.error(SpeechToTextErrors.recognizerNotAvailable.name, "Speech recognizer null", "")
                activeResult = null
            }
        }
        ...
        if (previousRecognizerLang == null ||
            ...
            handler.post {
                recognizerIntent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
                    ...
                    val localContext = pluginContext
                    if (null != localContext) {
                        putExtra(RecognizerIntent.EXTRA_CALLING_PACKAGE, localContext.applicationInfo.packageName)
                    }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray): Boolean {
        when (requestCode) {
            speechToTextPermissionCode -> {
                permissionToRecordAudio = grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED
                bluetoothDisabled = (grantResults.isEmpty() || grantResults.size == 1 || grantResults[1] != PackageManager.PERMISSION_GRANTED) || noBluetoothOpt
                completeInitialize()
                return true
            }
        }
    ...
    override fun onResults(results: Bundle) {
        val isFinal = true
        updateResults(results, isFinal)  // Fix: Dùng updateResults thay vì sendRecognitionResult

        // Fix: Logic continuous với check đúng
    ...
    override fun onEndOfSpeech() {
        ...
        // Fix: Logic continuous với check đúng và dummy result
        if (shouldContinuousListen && isListening()) {
            restartRunnable?.let { restartHandler.removeCallbacks(it) }
            restartRunnable = Runnable {
                try {
...
// Giữ nguyên LanguageDetailsChecker và ChannelResultWrapper từ code gốc (không thay đổi)
class LanguageDetailsChecker(flutterResult: Result, logging: Boolean) : BroadcastReceiver() {
    ...
    public fun createResponse(supportedLanguages: List<String>?) {
        ...
        if (null != supportedLanguages) {
            for (lang in supportedLanguages) {
                if (currentLocale.toLanguageTag() == lang) continue
    ...
    private fun debugLog(msg: String) {
        if (debugLogging) Log.d(logTag, msg)
    }
}
